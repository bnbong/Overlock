...
절차적 `SewingSkin`을 폴백으로 남겨 둔 채, 사용자 제공 재봉 UI 시트를 도입해 위젯을 실제
텍스처로 교체했다. 시트는 요소별로 분해해 `assets/gfx/ui/`에 개별 PNG로 저장하고(대형/소형
필 버튼·정사각 버튼 바탕, 베이지·다크 패널, 태그 라벨, 쉐브론, 진행 홈, 바늘 마커, 하단 아이콘
8종), 원본 시트는 `assets/gfx/src/ui_sheet_v1.png`(`.gdignore`)로 옮겼다.

- 로더 `scripts/ui/UiSkin.gd`: 9패치 `StyleBoxTexture`(버튼 — 캡의 단추/둥근 끝 보존 마진 실측),
  `NinePatchRect`(패널 — 모서리 실패·핀쿠션·패치 장식 보존 마진 실측), `TextureRect`(TIME 태그)로
  배선. 텍스처 미로드 시 모든 팩토리가 조용히 폴백해 `SewingSkin` 절차 스킨을 그대로 쓴다.
- 적용: 메인·맵 선택·설정·결과·일시정지의 필/아이콘 버튼과 카드 패널(베이지), 리더보드는 다크
  패널을 목록 배경으로 깔고 텍스트를 밝은 크림으로 전환(가독성). HUD는 TIME=태그 라벨, 속도
  게이지=쉐브론(상태별 modulate 틴트), 진행 바=원단 홈+은색 바늘 마커. 아이콘은 의미별 매핑
  (Play/◀▶/바늘/다운로드/휴지통/톱니/트로피).
- 절차 유지(판단): 미니맵·리스크 미터(장식이 게이지/궤적 가독성을 해침), 트랙 미리보기(작은
  실루엣을 다크 패널 장식이 가림), 에디터 툴바(좁은 텍스트 버튼은 필 버튼 양끝 캡이 글자를 밀어냄).
- 기능·레이아웃·판정·위젯 API·RaceDirector/HUD 배선 불변. 검증: gdlint 클린, 사본 `--import`
  에러 0, 비-headless 캡처(메인·맵 선택·설정·리더보드·주행 HUD·결과·일시정지), 헤드리스
  네비게이션 E2E(메인→맵 선택→주행 완주→Result→복귀) 에러 0.
- 결함 수정(대형 필 버튼 캡): 초기 배경 분리에서 `btn_large_normal`/`btn_large_pressed`의
  오른쪽 캡(단추+둥근 끝)이 크롭에 잘려 9패치 우측 마진이 뭉개진 단추를 렌더했다. 필 버튼은
  좌우 대칭이므로 깨끗한 왼쪽 절반을 수평 미러링해 오른쪽 캡을 복원하고, 알파 페더 halo를
  약하게 초크(부분 알파 13%→8%)했다. 텍스처 크기·9패치 마진·배선 불변(에셋 재베이크만).
- 재베이크 v2(사용자 신규 시트 6종 교체): 대형/소형 필 버튼·정사각 버튼·베이지/다크 패널을
  새 원본으로 갈아끼웠다. 원본이 여백을 크게 포함(예: 대형 2172×724 중 실요소 2059×423,
  베이지 1254×1254 중 1193×977)해 알파 bbox로 트림한 뒤 프리멀티플라이 알파 LANCZOS
  다운스케일로 재샘플했다. 9패치 코너는 텍스처 px가 화면 px로 1:1 렌더되므로(경험 검증),
  캡의 둥근 단추가 세로로 찌그러지지 않게 버튼 텍스처 높이를 화면 높이에 맞췄다 — 버튼
  273×56·190×48·정사각 98×96, 패널 베이지 573×469·다크 757×440. 9패치 마진 재실측:
  large 36/36/7/7, small 34/34/7/7, square 13, panel_beige 140/138/140/136,
  panel_dark 156/140/130/170(l/r/t/b). 러닝 스티치·모서리 장식이 세로로 긴 패널
  (Result·Leaderboard)에서 늘어져 번지지 않도록 패널 `NinePatchRect`와 버튼
  `StyleBoxTexture`에 `TILE_FIT` 축 스트레치를 적용(원단 필이 균일 — 그라디언트 없음 —
  이라 타일 이음매가 안 보인다). 알파 페더 halo는 엣지 국소 초크(a<9% → 0, a≥25% 불변)로
  제거해 다크 패널 내부 불투명도(≈0.99)는 보존. 버튼 텍스트 좌우 여백은 새 캡 폭에 맞춘
  하한(large 26·small 25·square 8px)을 둬 글자가 단추를 덮지 않게 했다. 텍스처·9패치·
  content margin·`TILE_FIT` 외 기능·배선 불변. 검증: gdlint 클린, 사본 `--import` 0,
  헤드리스 네비 E2E(메인→맵선택→설정→리더보드→완주→Result→맵선택) OK, 6화면 비-headless
  캡처 전/후 대조(캡 대칭·선명, 페더 halo 소멸, 세로 패널 무번짐 확인).
- 결함 수정(맵 선택 ◀▶ 나브 버튼): 트랙 변경 화살표에서 정사각 단추 바탕
  (`btn_square_blank` StyleBox)을 제거하고 화살표 아이콘만 남겼다. `UiSkin.skin_icon_nav`가
  배경 StyleBox 4상태를 `StyleBoxEmpty`로 덮어 투명 히트 영역(48×48)만 유지하고,
  hover=빨강·pressed=짙은 잉크로 아이콘 modulate 틴트만 피드백한다. `focus_mode=0`(순환 제외)은
  씬 그대로.
- 결함 수정(HUD TIME 태그 / 속도 게이지) — 위젯별 결정: 초기 추출본이 오염돼(태그=우측
  보라 리본 조각+회색 그림자, 쉐브론=드롭섀도 베이크로 틴트 탁화) 사용자가 지적했다.
  `UiSkin`에 위젯별 토글(`SKIN_TIME_TAG`/`SKIN_SPEED_CHEVRON`)을 두어 위젯 단위로 시트 스킨과
  절차 폴백을 오갈 수 있게 했다.
  - **TIME 태그 = 재추출 성공(텍스처 유지)**: 소스 시트에서 태그 몸통+빨간 리본만 타이트하게
    재크롭(x[34..418]·상단 버튼 잔재 제외, 우측 보라 리본 x≥418 제외)하고 faint alpha 초크로
    그림자 halo 제거 → `tag_label.png`(379×149). 확대 크롭에서 리본 조각·그림자 소멸 확인.
  - **속도 게이지 = 절차 원복(`SKIN_SPEED_CHEVRON=false`)**: 쉐브론을 무채색 정규화+드롭섀도
    제거로 재추출하니 1~3단은 깨끗해졌으나, 4~5단 점멸(`base*blink`, 저점 0.2배)에서 채워진
    직물 쉐브론이 거의 검은 덩어리로 뭉개져 사용자가 지적한 "탁한 덩어리" 증상이 위상 의존으로
    재현됐다(점멸은 텍스처·절차 공통 로직). 절차 폴리라인 쉐브론은 점멸 저점에서도 가느다란
    유채색 선으로 읽혀 사용자가 명시 허용한 원복 방향과 스펙(§4.4·§8.2)에 부합 → 게이지만
    절차로 되돌렸다(재추출본 `chevron.png`는 정리된 상태로 보존, 토글 시 즉시 재사용 가능).
  - 검증: gdlint/gdformat 클린, 사본 `--import` 에러 0, 헤드리스 네비 E2E(cotton) OK,
    비-headless 주행 캡처(HUD 전경+TIME·게이지 확대 크롭 전/후), 맵 선택 화살표 캡처.
    격리(사본 `config/name` 변경, `user://` 분리)로 실제 userdata 미접촉.
...
**눈 오버레이 스왑(현행 기본)**: `BackgroundFace`는 눈 없는 고정 베이스
(`face_base_clean.png`) 위에 상태별 눈 오버레이(`eyes_normal`/`eyes_focus`/`eyes_injured`)를
얹고, 상태 전환 시 **눈 오버레이만** 0.2s 상보 알파 디졸브한다(옛 오버레이 α=1→0, 새
오버레이 α=0→1). 텍스처는 `Gameplay.tscn`이 아니라 `BackgroundFace.gd`의 **스크립트
preload 기본값**으로 배선한다(씬 미수정). 부상(stun)→injured, 집중 강도(속도·리스크 통합
`tension ≥ 0.5`)→focus, 그 외→normal으로 상태를 결정하는 로직은 불변. 베이스가 매 프레임
동일·불투명이라 전환 내내 머리카락·헤어밴드·볼 외곽이 전혀 떨리지 않고(구 전체-얼굴 스왑의
윤곽 떨림 차단), 눈 오버레이가 투명 배경이라 눈 주변 밴드 경계가 사라진다. 눈은 앞머리
위에 그려지는 애니메이션 관례 구도이며 `eyes_offset_y`(세로 px)·`eyes_scale`(앵커 중점
`(640,248)` 기준 전체 배율) export로 PNG 재가공 없이 미세 조정한다(사용자 확정값
2026-08-22: `eyes_offset_y=50`·`eyes_scale=0.9` — 눈을 아래로 내리고 0.9배로 축소한 취향
확정값, 임의 롤백 금지). 눈 파츠는 소스 시트에서 각 눈을 자기 동공 중심 기준으로 확대해
(동공 앵커 `(454,248)`/`(826,248)` 고정) 레거시 `face_normal`의 홍채 폭(측정 ~105px)과
같거나 약간 큰 크기(~112px, +6%)로 가공했다 — 구 가공(~59px)이 "눈이 너무 작다"는 피드백을
받아 재가공. 땀방울은 `sweat.png` 스프라이트로 집중 강도에 비례해, 오른쪽 눈 바깥
관자놀이(눈 오버레이 rect 기준 `EYE_SWEAT_FRAC`)에 표시한다 — 앵커가 눈 오버레이 rect
기준이라 `eyes_offset_y`·`eyes_scale`을 바꾸면 땀 위치·크기가 눈을 그대로 따라온다(구 독립
`expr_sweat_frac` 앵커는 절차 폴백 전용으로만 잔존, 구 절차적 땀 대체).

새 얼굴 시트(`src/face_sheet_v2.png`)를 분해·정규화한 파이프라인, 눈 좌/우 판별 근거,
정규화 수치, 크로스페이드 계약은 `assets/gfx/README.md`의 "눈 오버레이 스왑" 절 참고.

*레거시*: 구 전체-얼굴 텍스처 스왑(`face_normal`/`face_focus`/`face_injured`, 표정 시트
`src/expressions_sheet_v1.png`를 `face_base` 위에 합성)은 `Gameplay.tscn`이 uid로 참조 중이라
제자리 보존하되 오버레이 모드가 우선해 미사용이다. `base_clean`을 비우면 전체-얼굴 스왑으로,
그 슬롯마저 비우면 단일 `face_base` + 절차적 라인아트로 순차 복귀한다(회귀 금지 경로).
...
- **얼굴(`BackgroundFace`)**: 새 얼굴은 앞머리가 눈(구 절차적 좌표 `0.31w/0.69w`, `0.33h`)을
  ...
  꺾기 트랜스폼과 도형 폴백은 그대로 보존. 이 절차적 오버레이와, 그 뒤 이 base 위에
  표정 시트를 합성한 전체-얼굴 텍스처 스왑(`face_normal`/`face_focus`/`face_injured`)은 모두
  하위 폴백으로 남고, **현행 우선 경로는 새 얼굴 시트(`src/face_sheet_v2.png`)로 만든 눈
  오버레이 스왑**이다(눈 없는 `face_base_clean` + `eyes_*`/`sweat`, 눈만 크로스페이드 →
  얼굴 윤곽 떨림·밴드 경계 차단; §12.4 · README).
...
- **배경/타이틀**: 재봉실 원화를 Lanczos로 1280×720 커버 업스케일(야간=`backdrop_room`,
  ...
  `TitleLabel`을 참조하지 않아 배선 변경 없이 교체됐고, 플라크·배경 배선은 그대로다. 단
  트랙 선택·미리보기는 §14.2의 맵 선택 화면으로 옮겼다.

### 14.2 메뉴 2단 네비게이션 (메인 → 맵 선택)

한 화면에 몰려 있던 메뉴를 두 단계로 쪼갰다. 메인(`Main.tscn`)은 타이틀 플라크와 세로
4버튼(Start / Settings / Leaderboard / Quit)만 두고, 트랙 캐러셀·미리보기·에디터 진입·
불러오기·삭제는 Start로 들어가는 맵 선택 화면(`TrackSelect.tscn`)으로 옮겼다. 화면 흐름은
`메인 → Start → 맵 선택 → 플레이 → 결과 → (Menu) → 맵 선택`이고, 맵 선택의 "뒤로"로
메인 4버튼으로 돌아간다. 결과 화면 "Menu"는 방금 플레이한 트랙이 살아 있는 맵 선택으로
이어져 최고 기록 갱신을 바로 확인하고 다른 트랙을 고르거나 재도전하게 했다.

- **키보드**: 메인은 ↑↓로 버튼 포커스를 옮기고 Enter로 실행한다(포커스는 빨간 테두리
  스타일박스로 표시). 맵 선택·리더보드는 ←→로 트랙을 순환하고 Esc로 뒤로 간다.
- **마지막 선택 트랙 기억**: 맵 선택을 떠날 때 현재 트랙 id를 `LeaderboardClient`의
  `user://settings.json`에 `last_track_id` 키로 적고(구조 확장은 키 추가뿐), 재진입 시
  복원한다. 삭제된 커스텀 트랙이면 첫 트랙으로 폴백한다.
- **리더보드 트랙 전환**: 리더보드 화면에 공식 트랙 ◀▶ 전환을 넣었다. 맵 선택의 "이 트랙
  리더보드"로 열면 해당 트랙으로 시작하고, 메인에서 직접 열면 첫 공식 트랙으로 자체
  초기화한다. 뒤로가기는 진입한 곳(`return_scene` 정적 변수)으로 돌아간다.
- **보존**: 설정 게이팅(온라인 URL·닉네임 조건), 커스텀 트랙 서버 제출/조회 제외, 웹
  가드(Quit 숨김·데스크톱 전용 불러오기)는 그대로다. 에디터 "뒤로"도 맵 선택으로 이어진다.
